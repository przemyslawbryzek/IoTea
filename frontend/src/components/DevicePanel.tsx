  ...
  return (
    <section>
      <div className="flex flex-col gap-4 p-5">
        {authenticated ? (
          <label className={`flex ${devices.length === 0 ? 'flex-col' : 'flex-row'} gap-2 text-sm font-medium justify-between`}>
            <span>Device</span>
            {devices.length === 0 ? (
              <div className="border border-black/25 px-4 py-3 text-sm w-full rounded">
                Install the mobile app to connect your device.
              </div>
            ) : (
              <select
